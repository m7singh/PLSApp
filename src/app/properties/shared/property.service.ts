...
import { IProperty, ISession } from './property.model'
...
export class PropertyService {
  ...
  getProperties():Observable<IProperty[]> {
    return this.http.get("/api/properties").map((response: Response) => {
        return <IProperty[]>response.json();
    }).catch(this.handleError);
  }

  getProperty(id:string):Observable<IProperty> {
    return this.http.get("/api/properties/" + id).map((response: Response) => {
        return <IProperty>response.json();
    }).catch(this.handleError);
  }
  ...
  saveProperty(property): Observable<IProperty> {
    ...
    return this.http.post('/api/properties', JSON.stringify(property), options).map((response: Response) => {
